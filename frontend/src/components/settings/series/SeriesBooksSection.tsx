...
export function SeriesBooksSection({ seriesId, seriesName, nodeType, isRoot, onMembershipsChanged }: Props) {
  ...
  return <section className="p-4 sm:p-5">
    ...
    <Dialog open={customising} title="Customise Reading Order" onClose={() => !saving && setCustomising(false)} className="max-w-2xl"><div className="p-5"><p className="mb-4 text-sm text-text-muted">Drag books into the preferred Reading order.</p><DraggableList books={[...ordered, ...unordered]} onChange={setOrdered} /><div className="mt-5 flex justify-end gap-2"><ActionButton onClick={() => setCustomising(false)}>Cancel</ActionButton><ActionButton variant="primary" onClick={() => void saveOrder()} disabled={saving}>Save custom order</ActionButton></div></div></Dialog>
    <Dialog open={removing !== null} title={`Remove from ${seriesName}?`} onClose={() => { setRemoving(null); setRemoveWarning(null); }} className="max-w-md"><div className="p-5"><p className="text-sm text-text-secondary">Remove “{removing?.title}” from this {isRoot ? "root collection" : "Series"}?</p>{removeWarning && <p role="alert" className="mt-3 rounded-lg border border-danger/30 bg-danger-muted/30 p-3 text-sm text-danger">{removeWarning}. Continuing removes those child assignments too.</p>}<div className="mt-5 flex justify-end gap-2"><ActionButton onClick={() => setRemoving(null)}>Cancel</ActionButton><ActionButton variant="danger" onClick={() => void confirmRemove(Boolean(removeWarning))}>{removeWarning ? "Remove from root and child Series" : "Remove"}</ActionButton></div></div></Dialog>
  </section>;
}
...
function ReadingSummary({ custom, onCustomise, onReset }: { custom: boolean; onCustomise: () => void; onReset: () => void }) { return <div className="mt-6 rounded-xl border border-border bg-surface-muted/30 p-4"><p className="font-medium text-text-primary">Reading Order</p><p className="mt-1 text-sm text-text-muted">{custom ? "Custom Reading order" : "Publication order (default)"}</p><div className="mt-3 flex gap-2"><ActionButton onClick={onCustomise}>{custom ? "Edit Reading Order" : "Customise Reading Order"}</ActionButton>{custom && <ActionButton variant="secondary" onClick={onReset}>Reset to Publication Order</ActionButton>}</div></div>; }
