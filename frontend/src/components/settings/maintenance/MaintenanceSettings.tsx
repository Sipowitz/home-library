...
import { cacheExistingCovers, cleanCoverCache, rescanAllCoverArt, getReviewQueue, startMaintenanceRefresh, getActiveMaintenanceJob, getMaintenanceJob, cancelMaintenanceJob, type MaintenanceJob } from "../../../api/maintenance";
...
function jobLabel(job: MaintenanceJob) {
  if (job.kind === "cover_rescan") {
    if (job.status === "completed") return "Cover art rescan complete";
    if (job.status === "failed") return "Cover art rescan failed";
    if (job.status === "cancelled") return "Cover art rescan cancelled";
    if (job.status === "pending") return "Cover art rescan queued";
    return "Rescanning cover art…";
  }
  ...
  if (job.kind === "cover_cache") return "Caching existing covers";
...
export function MaintenanceSettings({ active, reviewSaved, evidenceRefreshVersion = 0, onReview, onReviewSequenceComplete }: Props) {
  ...
  const [confirmKind, setConfirmKind] = useState<"metadata" | "cover_rescan" | "cover_cache" | "cover_cache_cleanup" | null>(null);
  ...
  async function startRefresh() {
    ...
    try {
      setJob(confirmKind === "cover_rescan" ? await rescanAllCoverArt() : confirmKind === "cover_cache" ? await cacheExistingCovers() : confirmKind === "cover_cache_cleanup" ? await cleanCoverCache() : await startMaintenanceRefresh(confirmKind));
    } catch (err: unknown) {
    ...
    <div className="max-w-6xl space-y-4">
      ...
      <section className="rounded-xl border border-border bg-surface/60 p-3 sm:p-4">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          <ActionButton variant="utility" disabled={Boolean(job && ["pending", "running"].includes(job.status))} onClick={() => setConfirmKind("cover_rescan")} className="px-3">Rescan All Cover Art</ActionButton>
      ...
      {job?.kind === "cover_rescan" && job.cover_rescan_counts && <p className="text-xs text-text-muted" aria-live="polite">{job.cover_rescan_counts.provider_lookups} provider lookups · {job.cover_rescan_counts.candidates_discovered} candidates found · {job.cover_rescan_counts.candidates_stored} stored locally · {job.cover_rescan_counts.failed_downloads} download failures · {job.cover_rescan_counts.provider_failures} provider failures · {job.cover_rescan_counts.skipped_no_isbn} without ISBN skipped</p>}
      {confirmKind && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 p-4" role="dialog" aria-modal="true"><div className="w-full max-w-md rounded-xl border border-border-strong bg-surface-raised p-5 text-text-primary shadow-2xl dark:bg-surface"><h2 className="text-lg font-semibold">{confirmKind === "cover_rescan" ? "Rescan all cover art?" : confirmKind === "cover_cache" ? "Cache existing covers?" : confirmKind === "cover_cache_cleanup" ? "Clean cover cache?" : "Start provider refresh?"}</h2><p className="mt-2 text-sm text-text-secondary">{confirmKind === "cover_rescan" ? "Check all enabled providers for each ISBN-bearing book and permanently store available artwork. Saved covers and metadata will not change. Books without ISBN are skipped." : confirmKind === "cover_cache" ? "Downloads externally hosted selected covers into local Library App storage. Failed covers remain unchanged, and this is safe to run again." : confirmKind === "cover_cache_cleanup" ? "Removes unused provider cache files and temporary cover files older than 24 hours. Selected, uploaded, and permanent covers are not removed." : "This may take several minutes. Saved Book fields and selected covers will not change; updated evidence may add books to the Review Queue."}</p><div className="mt-5 flex justify-end gap-2"><ActionButton variant="tertiary" onClick={() => setConfirmKind(null)}>Cancel</ActionButton><ActionButton variant="primary" onClick={startRefresh}>{confirmKind === "cover_rescan" ? "Start Rescan" : confirmKind === "cover_cache" ? "Cache Covers" : confirmKind === "cover_cache_cleanup" ? "Clean Cache" : "Start Refresh"}</ActionButton></div></div></div>}
    </div>
  );
}
