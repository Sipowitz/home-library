...
type DerivedBook = { book: Book; suggested: boolean };

function authorOrder(book: Book) {
  return book.author.trim().split(/\s+/).at(-1)?.toLocaleLowerCase() ?? "";
}

function mergeBooks(assignedBooks: Book[], suggestedBooks: Book[]): DerivedBook[] {
  return [
    ...assignedBooks.map((book) => ({ book, suggested: false })),
    ...suggestedBooks.map((book) => ({ book, suggested: true })),
  ].sort((left, right) => authorOrder(left.book).localeCompare(authorOrder(right.book)) || left.book.id - right.book.id);
}

function Books({ books, viewMode, locations, categories, showCovers, onSelect }: Omit<Props, "data"> & { books: DerivedBook[] }) {
  ...
  return viewMode === "grid"
    ? <BookGridView books={renderedBooks} suggestedBookIds={suggestedBookIds} onSelect={onSelect} />
    : <BookListView books={renderedBooks} suggestedBookIds={suggestedBookIds} locations={locations} categories={categories} showCovers={showCovers} onSelect={onSelect} />;
}

function LocationBranch({ group, depth, suggestionsByLocation, ...props }: Omit<Props, "data"> & { group: LocationBookGroup; depth: number; suggestionsByLocation: Map<number, SuggestedBook[]> }) {
...
export function GroupedLocationBooks({ data, ...props }: Props) {
  const suggestionsByLocation = new Map<number, SuggestedBook[]>();
