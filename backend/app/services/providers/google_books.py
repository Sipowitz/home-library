...
class GoogleBooksProvider(BookProvider):
    ...
    async def fetch_from_google(self, isbn: str) -> dict | None:
        url = f"{GOOGLE_BOOKS_URL}?q=isbn:{isbn}"

        if settings.GOOGLE_API_KEY:
            url += f"&key={settings.GOOGLE_API_KEY}"

        return await self.safe_request(url)

    async def fetch_book_by_isbn(self, raw_isbn: str) -> dict | None:
        ...
        data = await self.fetch_from_google(isbn)
        ...
        for item in data["items"]:
            ...
            for identifier in identifiers:
                if clean_isbn(identifier.get("identifier", "")) == isbn:
